<?php 
  include("init.php");
         ...
         if($from_date=='' or $to_date==''){$dd="and stock_product.stockin_date='$today'";} 
		  ...
		  ,tb_groups.Group_Name,tb_stock_product.stock_qty as total_qty
		   FROM  products
  ...
  on products.Product_ID=tb_stock_product.product_id
       ...
       where    1=1   $g_id  $p_id $name  group by products.Product_ID  order by products.Group_ID,products.Product_ID asc ");
		  if($sp){
   ...
   <input type="button"  class="btn btn-success btn-sm edit_pro"  id="<?php echo $s["Product_ID"]; ?>" value="ແກ້ໄຂ" data-toggle="modal" data-target="#add_pro" >
				...
				<td ><button class="btn btn-danger btn-sm delete_Id" value="<?php echo $s["Product_ID"]; ?>"  id="<?php echo  $s["Id"]; ?>">ລົບ</button></td>
               ...
               <input type="hidden" name="43_price" id="s4_price<?php echo  $s["Product_ID"]; ?>" class="form-control" value="<?php echo  $s["s4_price"]; ?>" />
