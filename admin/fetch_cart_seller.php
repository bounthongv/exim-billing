...
?>
	<table class="table table-bordered ">
		...
		<?php
if(!empty($_SESSION["cart_seller_order"]))
{      
         $e=1;
	foreach($_SESSION["cart_seller_order"] as $keys => $values)
	{
	...
	?>
       ...
       <input type="text" name="Price[]" style="text-align:center" onkeydown="return event.key != 'Enter';" class="form-control  btn-sm price" id="e_price<?=$e;?>"  value="<?=@number_format($values["product_price"],2);?>" onkeyup="amount()" ></td>	
            ...
            <input type="text" name="amount[]" style="text-align:right" class="form-control text_right btn-sm" id="amount<?=$e;?>"  value="<?=@number_format($values["product_quantity"] * $values["product_price"], 2);   ?>" readonly >
	...
	<input type="hidden" name="e_name<?=$values["Product_ID"];?>" id="e_name<?=$e;?>" value="<?=$values["product_name"];?>">
     ...
     <input type="text" class=" form-control text_right"  name="total_amount[]" id="total_amount" value="<?=@number_format($values["product_quantity"] * $values["product_price"], 2);?>" readonly="readonly" />
