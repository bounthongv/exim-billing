...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'update')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			...
			if($_POST["product_quantity"]>$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity'])
			{
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'];
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_price']=$_POST["product_price"];
				}
				else{
			$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$_POST["product_quantity"];			
			$_SESSION["cart_edit_sale_customer_order"][$keys]['product_price']=$_POST["product_price"];
				   }
			}
		}
	...
	if($_POST["action"] == 'update_x1')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			...
			if($_POST["qty"]>$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'])
			{
				//$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'];
				$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			    $_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$qty;
			
				$price = filter_var($_POST["price"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_price']=$price;
				}
				else{
			$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$qty;	
...
for ($i = 0; $i < count($_POST['item_list']); $i++) {
	...
	if($_POST["action"] == 'update_free')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			if($values["list_id"] == $_POST["Product_ID"])
			{
				...
				//$price = filter_var($_POST["price"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_price']='0';
				$_SESSION["cart_edit_sale_customer_order"][$keys]['status_free']='1';
	...
	if($_POST["action"] == 'update_plus')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			...
			$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']+1;
			/*if($_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']+1>$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'])
			{
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'];
	...
	if($_POST["action"] == 'update_minus')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			...
			if($_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']-1<1)
			{
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=0;
	...
	if($_POST["action"] == 'update_plus_c')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			...
			$_SESSION["cart_edit_sale_customer_order"][$keys]['crate_qty']=$_SESSION["cart_edit_sale_customer_order"][$keys]['crate_qty']+1;
			/*if($_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']+1>$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'])
			{
				$_SESSION["cart_edit_sale_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_sale_customer_order"][$keys]['qty_limit'];
	...
	if($_POST["action"] == 'update_minus_c')
	{
		foreach($_SESSION["cart_edit_sale_customer_order"] as $keys => $values)
		{
			...
			if($_SESSION["cart_edit_sale_customer_order"][$keys]['crate_qty']-1<1)
			{
				$_SESSION["cart_edit_sale_customer_order"][$keys]['crate_qty']=0;
