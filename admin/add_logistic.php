...
<div class="container">
    <br>
    <h3 align="center">ຈັດຖ້ຽວສິນຄ້າ</h3><br>
  </div> 
...
<!--</form>-->
    ...
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_logistic.php" method="post"  onkeydown="return event.key != 'Enter';" onsubmit="return ch_qty()" enctype="multipart/form-data" name="me">

	<div class="form-group row">
    <div class="col-sm-10">
    <a href="index.php"><button type="button" name="close"  class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a>
     <a href="cart_logistic.php?action=empty" ><button type="button" class="btn btn-success"><i class="fa fa-plus-square"></i>&nbsp;ເພີ່ມໃໜ່</button></a>
      
      <button type="submit" name="save"  class="btn btn-primary" ><i class="fa fa-file"></i>&nbsp;ບັນທືກ</button>
      
      <a href="cart_logistic.php?action=empty" ><button type="button" name="reset" value="reset" class="btn btn-warning"><i class="fa fa-trash-o"></i>&nbsp;ລືບ</button></a>
     </div>
     </div>
...
<table border="0">
  ...
  <tr>
    <td align="center">ເລກທີ: <br><input type="text" class="form-control ss" name="lg_id" id="lg_id" value="<?php echo $s_id; ?>" readonly ></td>
    <td align="center">ວັນທີ: <br><input type="date" class="form-control" name="lg_date" id="lg_date" value="<?php echo @date('Y-m-d');  ?>"  required> </td>
 ...
 <tr>
  ...
  <tr>
    ...
    <input type="text" class="form-control" name="regis_id"   id="regis_id" required   >
    
    </td>
     <td align="center">ຜູ້ຂັບ: <br><input type="text" name="driver_name" id="driver_name" class="form-control" required >
     </td>
     <td align="center">ການເງີນ: <br><input type="text" name="accounting_name" id="accounting_name" class="form-control" required >
     </td>
     <td align="center">ຂົນເຄື່ອງ: <br><input type="text" name="worker_name" id="worker_name" class="form-control" required >   
...
<table >
	<tr>
        ...
        <i class="fa fa-cart-plus"></i> ລາຍການສັ່ງຊື້</button>
        </td>
...
<div class="modal" id="select_product">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">ລາຍການຫີບຫໍ່ສິນຄ້າ</h4>
       ...
       &nbsp;  <button type="button" class="btn btn-danger" data-dismiss="modal">ປິດ</button>
        ...
        <div class="modal-body">
         <table >
          ...
          <table>
          <tr>
          <td>ລະຫັດຖ້ຽວ </td><td><input type="text" name="order_id" id="order_id" class="form-control " ></td>
          <td align="center" >ສາຍທາງ </td><td>
    <select name="route_id2" id="route_id2" class="form-control " required>    
    <option value="">ທັງໝົດ</option>	
    <?PHP 
	...
	<?PHP } ?>
         ...
         <td>ວັນທີ </td><td><input type="date" name="from_date" id="from_date" class="form-control" ></td>
        <td>  ຫາວັນທີ</td><td> <input type="date" name="to_date" id="to_date" class="form-control" > </td>
        <td>&nbsp;  </td><td> <button type="button" name="search" id="search" class="btn btn-primary s_order_list" ><i class="fa fa-search"></i> ຄົ້ນຫາ</button> </td>
          </tr>
          </table>
 ...
 function click_get_list()
	{
		...
		var action = 'show';
			$.ajax({
			url:"fetch_product_order_list.php",
			method:"POST",
			data:{  route_id:route_id,action:action },
			success:function(data)
			{
				$('#display_product').html(data);
				$('#route_id2').val(route_id);
			}
		});
	}
...
$(document).on('keyup', '.customer_id_s', function(){
  ...
  $(document).on('click', '.s_order_list', function(){
		...
		var action = 'show';
			...
			url:"fetch_product_order_list.php",
			method:"POST",
			data:{  order_id:order_id,route_id:route_id,action:action,from_date:from_date,to_date:to_date },
			success:function(data)
			{
				$('#display_product').html(data);
				$('#route_id2').val(route_id);
	...
	//	alert(customer_name);
		...
		var action = "add";
			...
			$.ajax({
				url:"cart_logistic.php",
				method:"POST",
				data:{   sale_id:sale_id,customer_name:customer_name,action:action },
				success:function(data)
				{
					$('#error').html(data);
					load_order_receipt();
