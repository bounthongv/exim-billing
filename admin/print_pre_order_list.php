...
<?php
       ...
       @$stock_id= mysqli_real_escape_string($con,$_GET['stock_id']);	
         if($stock_id==''){$s_id="";}  else{ $s_id="and pre_orders.stock_id='$stock_id'  ";}
		   ...
		   $today=date("Y-m-d");
         if($from_date=='' or $to_date==''){$btw="and pre_orders.sale_date='$today'";} 
		  else{ $btw="and pre_orders.sale_date between '$from_date' and '$to_date' ";}
		  
 
           @$sale_id= mysqli_real_escape_string($con,$_GET['sale_id']);		   
		 if($sale_id==''){$r_id="";}  else{ $r_id="and  pre_orders.sale_id='$sale_id' ";}
		 
		 

		  
		  @$sp=mysqli_query($con,"
  		select pre_orders.*,sum(pre_orders.total_amt) as t_total_amt,count(pre_orders.total_qty) as total_item
		,sum(pre_orders.amount) as total_amt,sum(pre_orders.discount) as total_dis  from 	  
   (SELECT pre_orders.*,sum(pre_orders.amount) as total_amt,sum(pre_orders.qty) as total_qty
   ,stocks.stock_name,products.Product_Name,products.size,products.Unit 
	   ...
	   left join customers on customers.customer_id=pre_orders.customer_id
       left join tb_groups on tb_groups.Group_ID=products.group_id
       
       where 1=1   $s_id $btw $r_id
         group by pre_orders.sale_id,pre_orders.product_id ) 
       as pre_orders
          
	   group by pre_orders.sale_id order by pre_orders.sale_id 
...
<body onLoad="print()" >
  ...
  <table width="800px"  align="center" >
    ...
    <td width="300px" align="center"><h6>ລາຍການໃບສັ່ງຊື້</h6><br><h7>ປະຈຳວັນທີ &nbsp;<?php $date=date_create("$from_date");
