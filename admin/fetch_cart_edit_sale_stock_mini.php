...
?>
	...
	<table class="table" align="center" style="max-width:1250px; " >
      ...
      <th align="center" >ຮູບ</th> 
		...
		<?php
if(!empty($_SESSION["cart_edit_sale_stock_mini"]))
{      
         $e=1;
	...
	?>
       ...
       <td align="center" ><img src="<?=$values["pic"];?>" height="60" /></td>
		    <td ><?=$values["Product_ID"];?> &nbsp; <?=$values["product_name"];?></td>
            <td align="center"><?=$values["units"];?></td>	
			<td align="center">
        <button type="button" class="btn btn-danger btn-sm qty_minus" data-Product_ID="<?=$values["Product_ID"];?>">-</button>
 <input type="text" name="QTY[]" style="text-align:center; max-width:40px; "   id="e_qty<?=$values["Product_ID"];?>"  class="qtu_box qty_enters "
        value="<?=$values["product_quantity"];?>"   data-Product_ID="<?=$values["Product_ID"];?>"  onkeyup="amount()" >
       <button type="button" class="btn btn-success btn-sm qty_plus" data-Product_ID="<?=$values["Product_ID"];?>">+</button>
       </td>
       	
	   <td align="right"><?=@number_format($values["product_price"],0);?>
       <input type="hidden" name="Price[]" style="text-align:right; max-width:80px;"  class="form-control  btn-sm price qty_enters" id="e_price<?=$values["Product_ID"];?>"  value="<?=@number_format($values["product_price"],0);?>" onkeyup="amount()" data-Product_ID="<?=$values["Product_ID"];?>" ></td>	
	   
					
			<td align="right" >
            
            <input type="text" name="amount[]" style="text-align:right;max-width:100px;" class="form-control " id="amount<?=$values["Product_ID"];?>"  value="<?=@number_format($values["product_quantity"] * $values["product_price"], 0);   ?>" readonly >
   ...
   <td align="right" >
            ...
            <input type="text" name="amount_crate[]" style="text-align:right;max-width:100px;" class="form-control " id="amount_crate<?=$values["Product_ID"];?>"  value="<?=@number_format($values["product_quantity"] * $values["crate_price"], 0);   ?>" readonly >
            </td>
    ...
    <!--<button type="button" name="delete"  class="btn btn-danger btn-sm delete_or" id="<?= $values["Product_ID"];?>" 
	...
	<input type="hidden" name="eee"  class="btn btn-success btn-sm  edit_cart_qty" id="<?=$values["Product_ID"];?>" data-toggle="modal" data-target="#edit_cart" value="ແກ້ໄຂ" >
			...
			<td align="center"> 
             <input type="text" name="total_amount_crate[]"id="total_amount_crate<?=$values["Product_ID"];?>"
             value="<?=@number_format(($values["product_quantity"] * $values["product_price"])+($values["product_quantity"] * $values["crate_price"]),0); ?>" 
             readonly="readonly"   class="form-control"  style="text-align:right;"  ></td>	
		</tr>
		<?php
	...
	<tr>  
        ...
        <td align="right"><strong><input type="text" class="btn btn-sm" readonly="readonly" name="total_qty" id="total_qty" 
        value="<?=number_format($total_qty,0);?>" style="text-align:right; max-width:120px;" />
        </strong></td> 
        <td></td>
      ...
      <input type="hidden" class="btn btn-sm" readonly="readonly" name="total_all_amount" id="total_all_amount" value="<?=number_format($total_price,0);?>" />
         ...
         <td align="right"><strong><input type="text" class="btn btn-sm" readonly="readonly" name="total_crate_price" id="total_crate_price" value="<?=number_format($total_crate_price,0);?>" style="text-align:right; max-width:120px;" />
      
        </strong></td> 
        
        <td align="right"><strong><input type="text" class="btn btn-sm" readonly="readonly" name="last_total" id="last_total" value="<?=@number_format($total_price+$total_crate_price,0);?>" style="text-align:right; max-width:120px;" />
