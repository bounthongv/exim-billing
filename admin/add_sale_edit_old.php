...
<!--</form>-->
    ...
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_sale_edit.php" method="post" onsubmit="return ch_qty()" enctype="multipart/form-data" name="me">
    ...
    <a href="index.php"><button type="button" name="close"  class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a>
      ...
      <a href="cart_sale_edit_add.php?action=empty" ><button type="button" name="reset" value="reset" class="btn btn-warning"><i class="fa fa-trash-o"></i>&nbsp;ລືບ</button></a>
...
<table border="0">
  ...
  <tr>
   ...
   <td align="right">ວັນທີ:</td>
    <td ><input type="date" class="form-control" name="sale_date" id="sale_date" value="<?php if(isset($_SESSION["sale_date"])){ echo $_SESSION["sale_date"];}else{ echo @date('Y-m-d'); } ?>" required> </td>
  </tr>
 <tr>
  ...
  <tr>
    ...
    <td>
    <input type="text" class="form-control ss" name="customer_name" id="customer_name" value="<?php if(isset($_SESSION["customer_id"])){ echo $_SESSION["customer_name"].'&nbsp;'.$_SESSION["customer_id"]; } ?>" >
    <input type="hidden" class="form-control" name="customer_id"   id="customer_id" value="<?php if(isset($_SESSION["customer_id"])){ echo $_SESSION["customer_id"]; }else{ echo 'C0001';} ?>"  >
   <!-- <input type="hidden" class="form-control" name="customer_type"   id="customer_type"  >-->
   
   
    </td>
     <td><button type="button" name="cc" class="btn btn-sm btn-info" data-toggle="modal" data-target="#customer_add" ><i class="fa fa-search"></i></button> ລາຄາ</td>  
     <td><select name="price_type" id="price_type" class="form-control" required>    	
   <?php if(isset($_SESSION["customer_type"])){ 
	    
       if($_SESSION["customer_type"]=='001'){ echo ' <option value="001">ຂາຄາຂາຍຍົກ</option>';}
   elseif($_SESSION["customer_type"]=='002'){ echo ' <option value="002">ລາຄາຂາຍພິເສດ</option>';}
   elseif($_SESSION["customer_type"]=='003'){ echo ' <option value="003">ລາຄາຂາຍ</option>';}
...
<table  border="0" >
  <tr>
    <td  align="right">ລວມູນຄ່າຕ້ອງຊຳລະ:</td>
    <td  align="right">
  ...
  <tr>
    ...
    <td align="right">
    <input type="text" name="payment" id="payment" onkeyup="key_press()" value="<?php if(isset($_SESSION["payment"])){ echo number_format($_SESSION["payment"],2);} ?>"  required class="form-control sinput">
      </td>
  </tr>
  <tr>
    <td align="right">ຍອດເຫຼືອ:</td>
    <td align="right"><input type="text" name="over_pay" id="over_pay" class="form-control sinput"  readonly></td>
  </tr>
  <tr>
    <td align="right">ມູນຄ່າຕ້ອງທອນ:</td>
    <td align="right"><input type="text" name="change" id="change" class="form-control sinput" readonly></td>
  </tr>
</table>
</td>
<td valign="top">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <tr>         
         ...
         <td><a href="add_sale_edit.php?c_id=<?=$f['customer_id'];?>&c_name=<?=$f['customer_name'];?>&c_t=<?=$f['customer_type'];?>"><button type="button" name="cc" class="btn btn-sm btn-success" >ເລືອກ</button></a></td>
        </tr>
        <?php } ?>
...
barcode_key.addEventListener("keyup", function(event) {
  ...
  // document.getElementById("myBtn").click();
	...
	var action = 'add'; 
			 ...
			 $.ajax({
				url:"cart_barcode_sale_edit.php",
				method:"POST",
...
function amount() {
    ...
    //   return false;
	  ...
	  $('#total').val(numeral(sum).format('0,000'));
