...
include('init.php');
  ...
  $barcode=mysqli_real_escape_string($con,$_POST['barcode']);   
       ...
       where    1=1 and  stock_product.qty >0    and products.Bar_Code='$barcode' and stock_product.stock_id='0001'
...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "add")
	{
		...
		if(isset($_SESSION["cart_quotation_trnasfer_mini_stock"]))
		{
			...
			{
				if($_SESSION["cart_quotation_trnasfer_mini_stock"][$keys]['Product_ID'] == $product_id)
				{
					$is_available++;
					
				if($_SESSION["cart_quotation_trnasfer_mini_stock"][$keys]['product_quantity'] == $qty_limit)
					{  
					// header("location:add_quotation_add_quotation_transfer_mini_stock.php?qty=$qty_limit");  
					
					 }
					else{	
			$_SESSION["cart_quotation_trnasfer_mini_stock"][$keys]['product_quantity'] = $_SESSION["cart_quotation_trnasfer_mini_stock"][$keys]['product_quantity'] + $qty;			
			  //  header("location:add_quotation_add_quotation_transfer_mini_stock.php?qty=$qty_limit");
					}
				}
			}
			if($is_available == 0)
			{
				...
				//header("location:add_quotation_add_quotation_transfer_mini_stock.php");
			}
		}
		else
		{
