...
<!--</form>-->
    ...
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_edit_sale_stock.php" method="post"  onkeydown="return event.key != 'Enter';" onsubmit="return ch_qty()" enctype="multipart/form-data" name="me">

	<div class="form-group row">
    <div class="col-sm-10">
    <a href="index.php"><button type="button" name="close"  class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a>
     <a href="cart_edit_sale_stock_mini.php?action=empty" ><button type="button" class="btn btn-success"><i class="fa fa-plus-square"></i>&nbsp;ເພີ່ມໃໜ່</button></a>
      
      <button type="submit" name="save"  class="btn btn-primary" ><i class="fa fa-file"></i>&nbsp;ບັນທືກ</button>
      
      <a href="cart_edit_sale_stock_mini.php?action=empty" ><button type="button" name="reset" value="reset" class="btn btn-warning"><i class="fa fa-trash-o"></i>&nbsp;ລືບ</button></a>
     </div>
     </div>
...
<table border="0">
     <input type="hidden" class="form-control ss" name="sale_id" id="sale_id" value="<?php echo $_SESSION['edit_sale_id']; ?>" >
  <tr>
    <td align="center">ເລກທີ: <br><input type="text" class="form-control ss" name="refer_no" id="refer_no" value="<?php echo $_SESSION['refer_no']; ?>" ></td>
   <td align="center">ວັນທີ: <br><input type="date" class="form-control" name="sale_date" id="sale_date" value="<?php echo @$_SESSION["sale_date"];  ?>"  required> </td>
 ...
 <tr>
  ...
  <tr>
    ...
    <input type="text" class="form-control ss" name="customer_name" id="customer_name" value="<?php echo @$_SESSION["customer_name"]; ?>" required >      
   ...
   <!-- <input type="hidden" class="form-control" name="customer_type"   id="customer_type"  >-->
     ...
     <td align="center">ປະເພດລູກຄ້າ: <br>
     <input type="text" name="customer_type" id="customer_type" class="form-control"  value="<?php echo @$_SESSION["ct_name"]; ?>"required readonly>   
     <input type="hidden" name="price_type" id="price_type" class="form-control" value="<?php echo @$_SESSION["ct_id"]; ?>" required>    	
...
$(document).on('keyup', '.customer_id_s', function(){
 ...
 function make_order_list()
	{
		...
		var action ='make_cart';
			...
			$.ajax({
				url:"cart_edit_sale_stock_mini.php",
			method:"POST",
 ...
 function load_order_receipt()
	{
			$.ajax({
			url:"fetch_cart_edit_sale_stock_mini.php",
			method:"POST",
			//dataType:"json",
			success:function(data)
			{
