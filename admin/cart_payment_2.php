...
if(isset($_POST["action"]))
{
	...
	{
		//
		if(isset($_SESSION["cart_payment_2"]))
		{   		
		unset($_SESSION["cart_payment_2"]);
		}
...
for ($i = 0; $i < count($_POST['item_list']); $i++) {
  ...
  tb_bank.Bank_Name
		 ...
		 where 1=1 and customer_payment.payment_type='2' and customer_payment.sale_id='$sale_id' order by payment_date asc
		...
		$f=mysqli_fetch_array($sql_d);
		  ...
		  echo $e_list;
	       $item_array = array(
			...
			$_SESSION["cart_payment_2"][] = $item_array;
    
	 
	
    
    }
	  header("location:add_payment_2.php");
	...
	{
		foreach($_SESSION["cart_payment_2"] as $keys => $values)
		{
			if($values["list_id"] == $_POST["Product_ID"])
			{
		     ...
		     $_SESSION["cart_payment_2"][$keys]['total']=$total;
	...
	if($_POST["action"] == 'remove')
	{
		foreach($_SESSION["cart_payment_2"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["cart_payment_2"][$keys]);
			}
		}
	}
...
if(isset($_GET["action"]) && $_GET["action"] == 'close_and_clear')
{
    ...
    header("location:payment_list_2.php");
    exit;
}
