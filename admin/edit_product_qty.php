...
<div class="container">
 ...
 <div class="container">   
  ...
  <a href="delete_product_qty.php?Id=<?php echo $_GET['Id']; ?>" onclick="return confirm('ທ່ານ ຕ້ອງການລົບແທ້ບໍ່?');"> 
   <button  type="button" class="btn btn-warning  " name="del" id="del"><i class="fa fa-trash"></i> ລົບ</button></a>
    </div>
  </div>
...
<table border="0">
  <tr>
    ...
    <td ><input type="text" class="form-control" name="product_id" id="product_id" value="<?php if(isset($_GET['product_id'])){ echo mysqli_real_escape_string($con,$_GET['product_id']);}else{} ?>" >  <input type="hidden" class="form-control" name="Id" id="Id" value="<?php if(isset($_GET['Id'])){ echo mysqli_real_escape_string($con,$_GET['Id']);}else{} ?>" ></td>
  ...
  <tr>
    ...
    <td><input type="text" class="form-control" name="price" id="price" value="<?php if(isset($_GET['price'])){ echo mysqli_real_escape_string($con,$_GET['price']);}else{} ?>"  ></td>
  </tr>
 <tr>
 ...
 <td align="right">ຈຳນວນ:</td>
    ...
    <input type="text" class="form-control" name="qty" id="qty" value="<?php if(isset($_GET['qty'])){ echo mysqli_real_escape_string($con,$_GET['qty']);}else{} ?>"  required> 
    
     </td>
     <td align="right">ວັນທີຫມົດອາຍຸ:</td>
    <td >

    <input type="date" class="form-control" name="expert_date" id="expert_date" value="<?php if(isset($_GET['expert_date'])){ echo mysqli_real_escape_string($con,$_GET['expert_date']);}else{echo @date('Y-m-d'); } ?>" required> 
 ...
 <td align="right">ສາງ:</td>
   ...
   <select name="stock_id" id="stock_id" class="form-control" required>   
	...
	<?PHP } ?>
    <?PHP 
	 $sql=mysqli_query($con,"select * from stocks where stock_id !='$stock_id' order by stock_id");	
	while($f = mysqli_fetch_array($sql)){?>
		<option value="<?php echo $f['stock_id']?>"><?php echo $f['stock_id']?> &nbsp; <?php echo $f['stock_name']?></option>
	<?PHP } ?>
    </select> 
