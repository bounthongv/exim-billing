<?php 
  include("init.php");
       ...
       @$stock_id= mysqli_real_escape_string($con,$_POST['stock_id']);	
         if($stock_id==''){$s_id="";}  else{ $s_id="and orders.stock_id='$stock_id'  ";}
		   ...
		   $today=date("Y-m-d");
         if($from_date=='' or $to_date==''){$btw="and orders.sale_date='$today'";} 
		  else{ $btw="and orders.sale_date between '$from_date' and '$to_date' ";}
		  
 
           @$sale_id= mysqli_real_escape_string($con,$_POST['sale_id']);		   
		 if($sale_id==''){$r_id="";}  else{ $r_id="and  orders.sale_id='$sale_id' ";}
		 
		 

		  
		  @$sp=mysqli_query($con,"
		select orders.*,sum(orders.total_amt) as t_total_amt,count(orders.total_qty) as total_item
		,sum(orders.amount) as total_amt ,sum(orders.discount) as total_dis from 	  
   (SELECT orders.*,sum(orders.amount) as total_amt,sum(orders.qty) as total_qty
   ,stocks.stock_name,products.Product_Name,products.size,products.Unit 
       ...
       left join stocks on stocks.stock_id=orders.stock_id
	   left join customers on customers.customer_id=orders.customer_id
       left join tb_groups on tb_groups.Group_ID=products.group_id
       
       where 1=1   $s_id $btw $r_id
         group by orders.sale_id,orders.product_id ) 
       as orders
          
	   group by orders.sale_id order by orders.sale_id 
 		...
 		<table border="1"   class="table-bordered " >
           ...
           <?php
			...
			?>	<tr>
              ...
              <td align="right">
          <a href="print_order.php?sale_id=<?=$s["sale_id"];?>&sale_date=<?=$s["sale_date"];?>&sale_time=<?=$s["sale_time"];?>" target="_blank" ><button type="button" class="btn btn-warning btn-sm"><i class="fa fa-print"></i> ພິມບິນ</button></a></td>
