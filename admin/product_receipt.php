...
$(document).ready(function(){
 ...
 $(document).on('keyup', '.s_supplier', function(){
 ...
 function load_order_receipt()
	{
			$.ajax({
			url:"fetch_cart_product_receipt.php",
			method:"POST",
			//dataType:"json",
			success:function(data)
			{
...
$(document).on('keyup', '.enter_cal', function(event){
	 if(event.which == 13) {
		 ...
		 var crate_price =$('#crate_price'+Product_ID+'').val();
		  var remark =$('#remark'+Product_ID+'').val();		 	
		
					 			  		   
		
		var status ='1';
		var action = "update_key_enter";

			$.ajax({
				url:"cart_product_receipt.php",
				method:"POST",
         data:{  Product_ID:Product_ID,qty:qty,price:price,action:action,status:status,crate_price:crate_price,remark:remark },
				success:function(data)
				{
...
<div class="container">
    ...
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_product_receipt.php" method="post"  onkeydown="return event.key != 'Enter';" enctype="multipart/form-data" name="me">
    ...
    <a href="index.php"><button type="button" name="close"  class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a>
     ...
     <a href="cart_product_receipt.php?action=empty">
...
<table border="0">
  <tr>
    ...
    <td ><input type="text" class="form-control" name="receipt_id" id="receipt_id" value="<?php echo $receipt_id; ?>" readonly ></td>
    <td></td>
    <td align="right">ວັນທີ:</td>
    <td >

    <input type="date" class="form-control" name="receipt_date" id="receipt_date" value="<?php echo @date('Y-m-d');  ?>" required> 
  ...
  <tr>
    ...
    <td><input type="text" class="form-control" name="staff_id" id="staff_id" value="<?php echo $_SESSION['username']; ?>"  ></td>
  </tr>
 <tr>
  ...
  <td></td>
  <td>
<!--  <label><input type="radio" name="status_pay" value="1">ຊຳລະ</label> &nbsp;
  <label><input type="radio" name="status_pay" value="2" checked>ບໍ່ຊຳລະ</label>-->
  </td>
 ...
 <div class="modal" id="supplier_add">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <table>
             <td>ລະຫັດ<br><input  type="text" name="product_id" id="product_id" class="form-control s_supplier"  ></td> 
             <td>ຊື່ຜູ້ສະໜອງ<br><input  type="text" name="product_name" id="product_name" class="form-control s_supplier"  ></td> 
             <td>ເບີໂທ<br><input  type="text" name="phone" id="phone" class="form-control s_supplier"  ></td> 
