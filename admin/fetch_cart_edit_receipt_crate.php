...
<table class="table" align="center" style="max-width:1250px;">
    ...
    <?php
    // [จุดที่แก้ไข 1]: เช็กทั้ง 2 ชื่อ Session เพื่อรองรับทั้งหน้าเพิ่มปกติและแก้ไข
    $cart_session = null;
   if(!empty($_SESSION["cart_edit_receipt_crate"])) {
        $cart_session = $_SESSION["cart_edit_receipt_crate"];
    }
    ...
    {      
        ...
        foreach($cart_session as $keys => $values)
        {
    ?>
    <tr>
        ...
        <td align="right">
            ...
            <input type="hidden" name="Price[]" style="text-align:right; max-width:80px;" class="form-control btn-sm price qty_enters" id="e_price<?php echo $values["Product_ID"];?>" value="<?php echo @number_format($values["product_price"], 0);?>" onkeyup="amount()" data-Product_ID="<?php echo $values["Product_ID"];?>">
        </td>	
        <td align="right"> 
        ...
        <input type="hidden" name="total_amount_crate[]" id="total_amount_crate<?php echo $values["Product_ID"];?>" value="<?php echo @number_format(($values["product_quantity"] * $values["product_price"]) + ($values["product_quantity"] * $values["crate_price"]), 0); ?>" readonly="readonly" class="form-control" style="text-align:right;">
    ...
    <tr>  
        ...
        <td align="center">
            <strong>
                <input type="text" class="btn btn-sm" readonly="readonly" name="total_qty" id="total_qty" value="<?php echo @number_format($total_qty, 0);?>" style="text-align:center; max-width:120px;" />
            </strong>
        </td> 
        ...
        <td align="right">
            <strong>
                <input type="text" class="btn btn-sm" readonly="readonly" name="total_all" id="total_all" value="<?php echo number_format($total_price, 0);?>" style="text-align:right; max-width:120px;" />
        ...
        <input type="hidden" class="btn btn-sm" readonly="readonly" name="last_total" id="last_total" value="<?php echo @number_format($total_price + $total_crate_price, 0);?>" style="text-align:right; max-width:120px;" />
        <input type="hidden" class="btn btn-sm" readonly="readonly" name="total_crate_price" id="total_crate_price" value="<?php echo number_format($total_crate_price, 0);?>" style="text-align:right; max-width:120px;" />
    </tr>
    <?php
    }
    else
    ...
    <?php // [จุดที่แก้ไข 2]: แก้ไข <? เป็น <?php
    }
    ?>
</table>