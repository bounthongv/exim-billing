...
date_default_timezone_set("Asia/Bangkok");
 ...
 $sale_dd_tt=date_create("$sale_date $sale_time");
  ...
  $sale_date_time = date_format($sale_dd_tt,"Y-m-d H:i:s");
    ...
    $s_id = mysqli_real_escape_string($con,$_SESSION['sale_id']);
     $sale_id = mysqli_real_escape_string($con,$_SESSION['sale_id']);
...
if(isset($_SESSION['sale_id'])){
	 ...
	 $sql_q=mysqli_query($con,"select *  from product_sale where sale_id='$sale_id' ");	 
 ...
 $sql_up=mysqli_query($con," update stock_product set qty=qty+$qty where product_lot_id='$product_lot_id' and stock_id='$stock_id'  ");
	  ...
	  $sql=mysqli_query($con,"delete from product_sale where sale_id='$sale_id'");
	   $sql=mysqli_query($con,"delete from payment where sale_id='$sale_id'");
...
for ($i = 0; $i < count($_POST['Product_ID']); $i++) {
		  ...
		  while( $p = mysqli_fetch_array($sql_pl))
	{
		...
		values('$s_id','$sale_date','$sale_time','$refer_no','$customer_id','$stock_id','$Product_ID','$p[product_lot_id]','$Price','$x_qty','$total_1','$dis1','$all_dis','$total','$payment','$payment_thb','$payment_lak','$payment_usd','$kip_baht','$dollar_baht','$user_id','$remark','$plan_date') ");
  ...
  $sql=mysqli_query($con,"INSERT INTO payment (pay_id,pay_date,sale_id,sale_date,pay_lak,pay_thb,pay_usd,total,c_lak,c_thb,c_usd) 
   ...
   if($sql_in){
		   ...
		   if(isset($_SESSION['smg'])){    }
		       ...
		       header("location:print_invoice.php?sale_id=$sale_id&sale_date=$sale_date");
		}
