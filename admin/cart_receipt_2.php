...
if(isset($_POST["action"]))
{
	...
	{
		//
		if(isset($_SESSION["cart_receipt_2"]))
		{   		
		...
		$f=mysqli_fetch_array($sql_d);
		  ...
		  echo $e_list;
	       $item_array = array(
			...
			$_SESSION["cart_receipt_2"][] = $item_array;
    
	 
	
    
    }
	  header("location:add_receipt_2.php");
...
/* ,product_sale.qty_p */
   ...
   ,stocks.stock_name,products.Product_Name,products.size,products.Unit 
    ...
    exit; // *** ตัด header("location:add_receipt_2.php"); ออก ***
}
	...
	{
		foreach($_SESSION["cart_receipt_2"] as $keys => $values)
		{
			if($values["list_id"] == $_POST["Product_ID"])
			{
		     ...
		     $_SESSION["cart_receipt_2"][$keys]['total']=$total;
	...
	if($_POST["action"] == 'remove')
	{
		foreach($_SESSION["cart_receipt_2"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["cart_receipt_2"][$keys]);
			}
		}
	}
