...
<div class="container">
    <br>
    <h3 align="center">ຂາຍສີນຄ້າ</h3><br>
  </div> 
...
<!--</form>-->
    ...
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_sale_customer_order.php" method="post"  onkeydown="return event.key != 'Enter';" onsubmit="return ch_qty()" enctype="multipart/form-data" name="me">

	<div class="form-group row">
    <div class="col-sm-10">
    <a href="index.php"><button type="button" name="close"  class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a>
     <a href="cart_sale_customer_order.php?action=empty" ><button type="button" class="btn btn-success"><i class="fa fa-plus-square"></i>&nbsp;ເພີ່ມໃໜ່</button></a>
      
      <button type="submit" name="save"  class="btn btn-primary" ><i class="fa fa-file"></i>&nbsp;ບັນທືກ</button>
      
      <a href="cart_sale_customer_order.php?action=empty" ><button type="button" name="reset" value="reset" class="btn btn-warning"><i class="fa fa-trash-o"></i>&nbsp;ລືບ</button></a>
     </div>
     </div>
...
<table border="0">
  ...
  <td align="center">ວັນທີຂາຍ: <br><input type="date" class="form-control" name="sale_date" id="sale_date" value="<?php echo @date('Y-m-d');  ?>"  required></td>
  <td align="center">ເວລາ: <br> <input type="time" class="form-control" name="sale_time" id="sale_time" value="<?php echo @date('H:i');  ?>"  required> </td>
    <td align="center">ວັນທີຕ້ອງການສົ່ງ: <br><input type="date" class="form-control" name="send_date" id="send_date" value="<?php echo @date('Y-m-d');  ?>" 
     required> </td>
   <td align="center">ເວລາ: <br>  <input type="time" class="form-control" name="send_time" id="send_time" value="<?php echo @date('H:i');  ?>"  required></td>
 ...
 <tr>
  ...
  <tr>
   ...
   <button type="button" name="cc" class="btn btn-sm " data-toggle="modal" data-target="#customer_add" onclick="get_customer()" ><i class="fa fa-search"></i></button> </span>   
    ...
    <td><input type="radio" name="status_payment" value="2" required> ເງີນສົດ
     <input type="radio" name="status_payment" value="3" required> ເງີນໂອນ <br>
     <br><input type="radio" name="status_payment" value="1" required > ຕິດໜີ້</td>
...
<div class="modal" id="select_product">
    <div class="modal-dialog">
      <div class="modal-content">
       ...
       <form action="cart_sale_customer_order.php" method="post" enctype="multipart/form-data">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <table>
             <td>ເລກທີ<br><input  type="text" name="order_id_ss" id="order_id_ss" class="form-control customer_id_ss"  ></td> 
             <td>ລະຫັດ<br><input  type="text" name="customer_id_ss" id="customer_id_ss" class="form-control customer_id_ss"  ></td> 
             <td>ຊື່ລູກຄ້າ<br><input  type="text" name="customer_name_ss" id="customer_name_ss" class="form-control customer_id_ss"  ></td> 
             <td>ເບີໂທ<br><input  type="text" name="customer_phone_ss" id="customer_phone_ss" class="form-control customer_id_ss"  ></td> 
 ...
 function click_get_list()
	{
		...
		var price_type = $('#price_type').val();
			$.ajax({
			url:"fetch_product_sale_customer_order_list.php",
			method:"POST",
			data:{  stock_id:stock_id,price_type:price_type },
			success:function(data)
			{
...
$(document).on('keyup', '.customer_id_s', function(){
	...
	$(document).on('keyup', '.customer_id_ss', function(){
	  
		var order_id = $('#order_id_ss').val();
		var customer_id = $('#customer_id_ss').val();
	    var customer_name = $('#customer_name_ss').val();
		var customer_phone = $('#customer_phone_ss').val();
		var action = "show";
		...
		$.ajax({
			url:"search_order.php",
			method:"POST",
		  data:{  action:action,customer_id:customer_id,customer_name:customer_name,customer_phone:customer_phone,order_id:order_id },
			success:function(data)
			{
				$('#show_order').html(data);
  ...
  $(document).on('click', '.add_customer', function(){
   ...
   $(document).on('click', '.add_order', function(){
        ...
        var route_id = $(this).attr("data-route_id");
		var route_name = $(this).attr("data-route_name");
		...
		$.ajax({
			...
			method:"POST",
			data:{  
			customer_id:customer_id,customer_name:customer_name,ct_id:ct_id,ct_name:ct_name,route_id:route_id,order_id:order_id,route_name:route_name,sr_id:sr_id,sr_fname:sr_fname,sr_lname:sr_lname },
			success:function(data)
			{
				//make_order_list();
				load_order_receipt();
 ...
 function make_order_list()
	{
		...
		var action ='reset';
		$.ajax({
				url:"cart_sale_customer_order.php",
			method:"POST",
			data:{ action:action },
			success:function(data)
			{
 ...
 function load_order_receipt()
	{
			$.ajax({
			url:"fetch_cart_sale_customer_order.php",
			method:"POST",
			//dataType:"json",
			success:function(data)
			{
 ...
 $(document).on('click', '.qty_plus', function(){
		...
		var action = "update_plus";
			...
			$.ajax({
				url:"cart_sale_customer_order.php",
				method:"POST",
         data:{  qty:qty,Product_ID:Product_ID,action:action,status:status,qty_limit:qty_limit },
				success:function(data)
				{
...
/*
$(document).on('click', '.qty_plus_c', function(){
 ...
 $(document).on('click', '.qty_minus_c', function(){
...
$(document).on('click', '.free_item', function(){
