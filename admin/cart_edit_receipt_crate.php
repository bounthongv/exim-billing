...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "add")
	{
		...
		{
			$is_available = 0;
			foreach($_SESSION["cart_edit_receipt_crate"] as $keys => $values)
			{   
			
				if($_SESSION["cart_edit_receipt_crate"][$keys]['Product_ID'] == $_POST["Product_ID"])
				{
										
					$is_available++;
					if($_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity'] == $_POST["qty_limit"])
					{  }
					else{
					
					$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity'] = $_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity'] +                      $_POST["product_quantity"];	
					
					$_SESSION["cart_edit_receipt_crate"][$keys]['product_price'] =   $_POST["product_price"];								 
					}
				}
	...
	if($_POST["action"] == 'remove')
	{
		foreach($_SESSION["cart_edit_receipt_crate"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["cart_edit_receipt_crate"][$keys]);
			}
		}
	}
	if($_POST["action"] == 'update')
	{
		foreach($_SESSION["cart_edit_receipt_crate"] as $keys => $values)
		{
			...
			if($_POST["product_quantity"]>$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity'])
			{
				$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$_SESSION["cart_edit_receipt_crate"][$keys]['qty_limit'];
				$_SESSION["cart_edit_receipt_crate"][$keys]['product_price']=$_POST["product_price"];
				}
				else{
			$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$_POST["product_quantity"];			
			$_SESSION["cart_edit_receipt_crate"][$keys]['product_price']=$_POST["product_price"];
				   }
			}
		}
	...
	if($_POST["action"] == 'update_x1')
	{
		foreach($_SESSION["cart_edit_receipt_crate"] as $keys => $values)
		{
			...
			if($_POST["qty"]>$_SESSION["cart_edit_receipt_crate"][$keys]['qty_limit'])
			{
				//$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$_SESSION["cart_edit_receipt_crate"][$keys]['qty_limit'];
				$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			    $_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$qty;
			
				$price = filter_var($_POST["price"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
				$_SESSION["cart_edit_receipt_crate"][$keys]['product_price']=$price;
				}
				else{
			$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$qty;	
  ...
  on products.Product_ID=tb_stock_product.product_id
	...
	if($_POST["action"] == 'update_plus')
	{
		foreach($_SESSION["cart_edit_receipt_crate"] as $keys => $values)
		{
			...
			$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']+1;
			/*if($_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']+1>$_SESSION["cart_edit_receipt_crate"][$keys]['qty_limit'])
			{
				$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=$_SESSION["cart_edit_receipt_crate"][$keys]['qty_limit'];
	...
	if($_POST["action"] == 'update_minus')
	{
		foreach($_SESSION["cart_edit_receipt_crate"] as $keys => $values)
		{
			...
			if($_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']-1<1)
			{
				$_SESSION["cart_edit_receipt_crate"][$keys]['product_quantity']=0;
