...
$(document).ready(function(){
 ...
 function load_list()
	{
			$.ajax({
			url:"fetch_summary_report_is_empty.php",
			method:"POST",
			//dataType:"json",
...
$(function(){
 ...
 //  alert(stock_id);
         ...
         $.ajax({
				url:"fetch_summary_report_is_empty.php",
				method:"POST",
				data:{  stock_id:stock_id,from_date:from_date,to_date:to_date,sale_id:sale_id,group_id:group_id,product_id:product_id },
				success:function(data)
				{
					$('#head_list').html(data);
...
$(document).on('click', '#print', function(){
   ...
   var group_id = $('#group_id').val(); 
	//	var action = $(this).attr("value");
     ...
     window.open('print_summary_report_is_empty.php?stock_id='+stock_id + '&from_date='+from_date  + '&to_date='+to_date+ '&product_id='+product_id+ '&group_id='+group_id+' ','_blank'); 
...
<div class="container">
    <br>
    <h3 align="center">ລາຍງານການຊື້ສົ່ງລັງ-ແກ້ວເປົ່າ</h3><br>
...
<table>
    ...
    <?PHP 
	 $sql=mysqli_query($con,"select * from stocks where office_id='$office1'");	
	...
	<?PHP } ?>
      ...
      <select name="group_id" id="group_id" class="form-control" required>   
       	<option value="">ທັງຫມົດ</option>
    <?PHP 
	 $sql=mysqli_query($con,"select * from tb_groups where office_id='$office1'");	
	while($f = mysqli_fetch_array($sql)){?>
