...
?>
	...
	<table class="table table-bordered ">
		...
		<?php
if(!empty($_SESSION["cart_edit_sale"]))
{      
         $e=1;
	foreach($_SESSION["cart_edit_sale"] as $keys => $values)
	{
	...
	?>
	   ...
	   <td>
       <input type="text" name="Price[]" style="text-align:center" onkeydown="return event.key != 'Enter';" class="form-control  btn-sm price" id="e_price<?=$e;?>"  value="<?=@number_format($values["product_price"],2);?>" onkeyup="amount()" ></td>	
	   
					
			<td align="right">
            
            <input type="text" name="amount[]" style="text-align:right" onkeydown="return event.key != 'Enter';" class="form-control text_right btn-sm" id="amount<?=$e;?>"  value="<?=@number_format($values["product_quantity"] * $values["product_price"], 2);   ?>" readonly >
	...
	<input type="hidden" name="e_name<?=$values["Product_ID"];?>" id="e_name<?=$e;?>" value="<?=$values["product_name"];?>">
	 ...
	 <td><input type="text" class=" form-control text_right"  name="discount[]" onkeydown="return event.key != 'Enter';"  id="discount" value="0" onkeyup="discount_d()" /></td>		
     <td>
     
     <input type="text" class=" form-control text_right"  name="total_amount[]" onkeydown="return event.key != 'Enter';" id="total_amount" value="<?=@number_format($values["product_quantity"] * $values["product_price"], 2);?>" readonly="readonly" />
	...
	<tr>  
        ...
        <td align="right"><strong><input type="text" class="btn btn-sm" readonly="readonly" name="total_all_amount" id="total_all_amount" value="<?=number_format($total_price, 2);?>" />
        </strong></td> 
        <td></td>  
    </tr>
	<?php
