...
';
if(!empty($_SESSION["cart_trnasfer_mini_stock"]))
{      
         $e=1;
	foreach($_SESSION["cart_trnasfer_mini_stock"] as $keys => $values)
	{
		...
		<tr>
			...
			<td>
       <input type="text" name="QTY[]" style="text-align:center"  class="form-control btn-sm qty_enters" id="e_qty'.$values["Product_ID"].'"  value="'.$values["product_quantity"].'"    onkeyup="amount()" autocomplete="off" data-Product_ID="'.$values["Product_ID"].'" ></td>	
	   <td>
       <input type="text" name="Price[]" style="text-align:center"  class="form-control  btn-sm" id="e_price'.$values["Product_ID"].'"  value="'.@number_format($values["product_price"],2).'" onkeyup="amount()" readonly="readonly"></td>	
	   
					
			<td align="right"><input type="text" name="amount[]" style="text-align:center" class="form-control  btn-sm" id="amount'.$values["Product_ID"].'"  value="'.@number_format($values["product_quantity"] * $values["product_price"],0).'" readonly ></td>
