     ...
     $stock_id=$_SESSION["stock_id"]; 
	  ...
	  @$price_type= mysqli_real_escape_string($con,$_POST['price_type']);		     
		  ...
		  elseif($price_type=='003'){ $pp=",products.s3_price  as price";}
		   ...
		   stock_product.product_lot_id ,stock_product.price ,sum(stock_product.qty) as qty  $pp          
		   FROM  products
...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "add")
	{
		...
		if(isset($_SESSION["cart_edit_sale"]))
		{
			...
			{
				if($_SESSION["cart_edit_sale"][$keys]['Product_ID'] == $product_id)
				{
					$is_available++;
					
				if($_SESSION["cart_edit_sale"][$keys]['product_quantity'] == $qty_limit)
					{   header("location:transfer_mini_stock_edit.php?qty=$qty_limit");   }
					else{	
			$_SESSION["cart_edit_sale"][$keys]['product_quantity'] = $_SESSION["cart_edit_sale"][$keys]['product_quantity'] + $qty;			
			 //   header("location:transfer_mini_stock_edit.php?qty=$qty_limit");
					}
				}
			}
