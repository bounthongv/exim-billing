...
<table  >
 ...
 <div class="container">   
    ...
    <div class="col-sm-10">
      ...
      <button type="submit" name="update" class="btn btn-primary" value="save"  ><i class="fa fa-file"></i>&nbsp;ບັນທືກ</button>
...
<table border="0">
  ...
  <tr>
    ...
    <td >
  <input type="text" class="form-control" name="transfer_id" id="transfer_id" value="<?php if($_SESSION["transfer_id"]==""){ echo $receipt_id; }else{ echo $_SESSION["transfer_id"];} ?>" readonly >
  </td>
   <td align="right">ວັນທີ:</td>
    <td ><input type="date" class="form-control" name="transfer_date" id="transfer_date" value="<?php if($_SESSION["transfer_date"]==""){ echo @date('Y-m-d'); }else{ echo $_SESSION["transfer_date"];} ?>" required> </td>
  </tr>
 <tr>
  ...
  <tr>
   ...
   <?php
   if($_SESSION['t_stock_id']==""){}
   else{
	echo '<option value="'.$_SESSION['t_stock_id'].'">'.$_SESSION['t_stock_id'].'&nbsp;'.$_SESSION['t_stock_name'].'</option> ';   
	   }
   ?>
    <?php
