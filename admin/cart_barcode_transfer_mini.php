...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "add")
	{
		...
		if(isset($_SESSION["cart_trnasfer_mini_stock"]))
		{
			...
			{
				if($_SESSION["cart_trnasfer_mini_stock"][$keys]['Product_ID'] == $product_id)
				{
					$is_available++;
					
				if($_SESSION["cart_trnasfer_mini_stock"][$keys]['product_quantity'] == $qty_limit)
					{   header("location:transfer_mini_stock.php?qty=$qty_limit");   }
					else{	
			$_SESSION["cart_trnasfer_mini_stock"][$keys]['product_quantity'] = $_SESSION["cart_trnasfer_mini_stock"][$keys]['product_quantity'] + $qty;			
			    header("location:transfer_mini_stock.php?qty=$qty_limit");
					}
				}
			}
