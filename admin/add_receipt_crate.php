...
$sql_max=mysqli_query($con,"select IFNULL(max(SUBSTRING(receipt_id,4, 6)),0) as m_id  from product_receipt_crate where year(receipt_date)='$id_y'   ");
...
if($max_id<1){    $s_id=$id1;     }
 ...
 else if($max_id<9){  $s_id=$year_id.'.'.'00000'.$id2;}  // 0000.2-0000.9
 else if($max_id<99){  $s_id=$year_id.'.'.'0000'.$id2;}  // 000.2-000.9

 else if($max_id<999){  $s_id=$year_id.'.'.'000'.$id2;} // 0010-00999  //   0100 - 999

  else if($max_id<9999){  $s_id=$year_id.'.'.'00'.$id2;} 
  else if($max_id<99999){  $s_id=$year_id.'.'.'0'.$id2;}
...
<div class="container">
    <br>
    <h3 align="center">ຊື້ຄືນລັງເປົ່າ</h3><br>
  </div> 
...
<!--</form>-->
    ...
    <!-- /.container -->
    	...
    	<form action="insert_receipt_crate.php" method="post"  onkeydown="return event.key != 'Enter';" onsubmit="return ch_qty()" enctype="multipart/form-data" name="me">
<div class="container"> 
	<div class="form-group row">
    <div class="col-sm-10">
    <a href="index.php"><button type="button" name="close"  class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a>
     <a href="cart_receipt_crate.php?action=empty" ><button type="button" class="btn btn-success"><i class="fa fa-plus-square"></i>&nbsp;ເພີ່ມໃໜ່</button></a>
      
      <button type="submit" name="save"  class="btn btn-primary" ><i class="fa fa-file"></i>&nbsp;ບັນທືກ</button>
      
      <a href="cart_receipt_crate.php?action=empty" ><button type="button" name="reset" value="reset" class="btn btn-warning"><i class="fa fa-trash-o"></i>&nbsp;ລືບ</button></a>
     </div>
     </div>
...
<table border="0">
  ...
  <tr> 
    <td align="center">ເລກທີ: <br><input type="text" class="form-control ss" name="receipt_id" id="receipt_id" value="<?php echo $s_id; ?>" readonly ></td>
   <td align="center">ວັນທີ: <br><input type="date" class="form-control" name="receipt_date" id="receipt_date" value="<?php echo @date('Y-m-d');  ?>"  required> </td>
...
$(document).on('keyup', '.customer_id_s', function(){
 ...
 function load_order_receipt()
	{
			$.ajax({
			url:"fetch_cart_receipt_crate.php",
			method:"POST",
			//dataType:"json",
			success:function(data)
			{
	 ...
	 if(event.which == 13) {
		...
		var action = "update_x1";
			...
			$.ajax({
				url:"cart_receipt_crate.php",
				method:"POST",
         data:{  Product_ID:Product_ID,qty:qty,action:action },
				success:function(data)
				{
