...
<?php
$sql_max=mysqli_query($con,"select IFNULL(max(SUBSTRING(transfer_id, 2, 5)),0) as id from transfer");
...
if($max_id<1){    $receipt_id=$id1;     }
 ...
 else if($max_id<9){  $receipt_id='T0000'.$id2;}  // 000.2-000.9
 else if($max_id<99){  $receipt_id='T000'.$id2;}  // 000.2-000.9

 else if($max_id<999){  $receipt_id='T00'.$id2;} // 0010-0099  //   0100 - 999

  else if($max_id<9999){  $receipt_id='T0'.$id2;} 
...
<div class="container">
    <br>
    <h3 align="center">ຮັບສີນຄ້າຈາກສາງລົດ</h3><br>
  </div> 

 
<div class="container">
<form action="cart_barcode_transfer_mini.php" method="post" enctype="multipart/form-data" name="bar">
<table  >
 <tr>
    ...
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_transfer_mini_stock.php" method="post"  onkeydown="return event.key != 'Enter';" onsubmit="return ch_qty()" enctype="multipart/form-data" name="me">
    ...
    <div class="col-sm-10">
      ...
      <a href="cart_transfer_from_mini_stock.php?action=empty" ><button type="button" name="reset" value="reset" class="btn btn-danger"><i class="fa fa-trash-o"></i>&nbsp;ລືບ</button></a>
 ...
 <tr>
  ...
  <tr>
   ...
   <select name="f_stock_id" id="f_stock_id" class="form-control" required>    	
    <?PHP 
	 $sql=mysqli_query($con,"select * from stocks where stock_id !='0001'");	
...
<div class="modal" id="select_product">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="submit" class="btn btn-success" name="add" value="ຕົກລົງ">
...
$(document).ready(function(){
 ...
 function load_order_receipt()
	{
			$.ajax({
			url:"fetch_cart_transfer_from_mini_stock.php",
			method:"POST",
			//dataType:"json",
			success:function(data)
			{
 ...
 $(document).on('click', '.add_pro', function(){
		...
		var action = "add";
			...
			$.ajax({
				url:"cart_transfer_from_mini_stock_add.php",
				method:"POST",
	 ...
	 $(document).on('click', '.delete_or', function(){
		...
		var action = "remove";
			...
			$.ajax({
				url:"cart_transfer_from_mini_stock_add.php",
				method:"POST",
				data:{   product_lot_id:product_lot_id,action:action },
				success:function(data)
				{
