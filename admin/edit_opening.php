...
$(document).ready(function(){
 ...
 function load_product()
	{
			$.ajax({
			url:"fetch_product_opening_list_add.php",
			method:"POST",
			//dataType:"json",
...
<?php
$sql_max=mysql_query("select max(SUBSTRING(receipt_id, 2, 6)) as id from product_receipt");
...
?>
<style>
.input{ width:180px; height:50px padding:4px; border:1px solid #D8D8D8; border-radius:4px;}
</style>
<div class="container">
    <br>
    <h3 align="center">ຍອດຍົກຈຳນວນສີນຄ້າ</h3><br>
  </div> 
<?php
 $Id=mysqli_real_escape_string($con,$_GET['Id']);
$sql=mysqli_query($con,"select stock_opening.*,products.Product_Name,products.Unit,products.ups
 from stock_opening
 LEFT JOIN products on products.Product_ID=stock_opening.product_id
  where stock_opening.Id='$Id'");
   
   $r=mysqli_fetch_array($sql);

    $dd=date_create($r['open_date']);
  $y= date_format($dd,'Y');
  $m= date_format($dd,'m');
?>
    <!-- /.container -->
 <div class="container">   
    	<form action="insert_product_opening.php" method="post" enctype="multipart/form-data" name="me">

	<div class="form-group row">
    <div class="col-sm-10">
     <a href="add_opening.php"> <button type="button" class="btn btn-success"><i class="fa fa-plus-square"></i>&nbsp;ເພີ່ມໃໜ່</button></a>
      <button type="submit" name="action" class="btn btn-primary" value="Update"><i class="fa fa-file"></i>&nbsp;ບັນທືກ</button>
     <a href="opening_list.php"> <button type="button" name="reset" value="reset" class="btn btn-danger"><i class="fa fa-trash-o"></i>&nbsp;ປິດ</button></a>
...
<table border="0">
  <tr>
    ...
    <td ><input type="text" class="form-control input" name="product_id" id="product_id" value="<?php echo $r['product_id']; ?>" >
    <input type="hidden"  name="Id" id="Id" value="<?php echo $r['Id']; ?>" >
    </td>
    <td align="right">ຊື່ສິນຄ້າ:</td>
    <td ><input type="text" class="form-control" name="product_name" id="product_name" value="<?php echo $r['Product_Name']; ?>" ></td>
    
  </tr>
  <tr>
    <td align="right">ຫົວໜ່ວຍ:</td>
    <td><input type="text" class="form-control" name="unit" id="unit" value="<?php echo $r['Unit']; ?>" ></td>
    <td align="right">ຂະໜາດບັນຈຸ</td>
    <td><input type="text" class="form-control" name="ups" id="ups"  value="<?php echo $r['ups']; ?>" ></td>
  </tr>
    <tr>
 
    <td align="right">ລາຄາຕົ້ນທືນ</td>
    <td><input type="text" class="form-control" name="price" id="price" onkeyup="total_amount()" required value="<?php echo $r['price']; ?>"  ></td>
    <td align="right">ຈຳນວນ:</td>
    <td ><input type="text" class="form-control" name="qty" id="qty" onkeyup="total_amount()" value="<?php echo $r['qty']; ?>"  required></td>
  </tr>
 
 <tr>
 
     <td align="right">ມູນຄ່າ:</td>
    <td >
    <input type="text" class="form-control" name="amount" id="amount" value="<?php echo $r['amount']; ?>" required>     
     </td>
   ...
   <select name="stock_id" id="stock_id" class="form-control" required>   
       <?PHP 
	 $sql=mysqli_query($con,"select * from stocks where stock_id='$r[amount]'");	
   ...
   <tr>
    ...
    <td align="right">ເດືອນ</td>
    <td> <select name="m" id="m" class="form-control">
    ...
    <td align="right">ປີ:</td>
    <td ><select name="y" id="y" class="form-control">
