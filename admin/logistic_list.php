...
<div class="container">
    <br>
    <h3 align="center">ລາຍການຈັດຖ້ຽວລົດ</h3><br>
...
<table>
     ...
     <td> <br>  <a href="index.php"> <button type="button" name="reset" value="reset" class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a></td>
       <td> <br><a href="add_logistic.php"  ><button type="button" class="btn btn-success" >
            <i class="fa fa-plus-square"></i>&nbsp; ຈັດຖ້ຽວລົດ</button></a></td>
...
$(document).on('keyup', '.customer_id_s', function(){
 ...
 //  alert(stock_id);
         ...
         $.ajax({
				url:"fetch_logistic_list.php",
				method:"POST",
				data:{  route_id:route_id,from_date:from_date,to_date:to_date,order_id:order_id,customer_id:customer_id },
				success:function(data)
				{
					$('#head_list').html(data);
...
$(document).on('click', '.delete_Id', function(){
  ...
  if (r == true) {
	...
	//	var action = $(this).attr("value");
window.open('print_customer_order.php?sale_id='+sale_id+' ','_blank');
	});
...
$(document).on('click', '.edit_Id', function(){
	...
	//	var action = $(this).attr("value");
     ...
     window.location = 'cart_edit_customer_order.php?order_id='+order_id ;
