...
<?php
if(!empty($_SESSION["cart_pre_order"]))
{
	foreach($_SESSION["cart_pre_order"] as $keys => $values)
	{
		...
		<tr>
			...
			<td>
            <input type="text" name="QTY[]" onkeyup="amount()" onkeydown="return event.key != 'Enter';" style="text-align:center" class="form-control edit_cart" 
                 id="<?=$values["Product_ID"];?>"  value="<?=$values["product_quantity"];?>" ></td>	
			<td align="right">
			<input type="text" name="Price[]" onkeyup="amount()" onkeydown="return event.key != 'Enter';" class="form-control edit_cart" 
