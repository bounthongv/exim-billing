...
$(function(){
 ...
 //  alert(stock_id);
         ...
         $.ajax({
				url:"fetch_sale_difference.php",
				method:"POST",
				data:{  stock_id:stock_id,from_date:from_date,to_date:to_date,sale_id:sale_id,group_id:group_id,product_id:product_id,customer_id:customer_id,user_id:user_id },
				success:function(data)
				{
					$('#head_list').html(data);
...
$(document).on('click', '#print', function(){
   ...
   var user_id = $('#user_id').val();
     ...
     window.open('print_sale_difference.php?stock_id='+stock_id+'&from_date='+from_date+'&to_date='+to_date+'&product_id='+product_id+'&group_id='+group_id+'&sale_id='+sale_id+'&customer_id='+customer_id+'&user_id='+user_id+'','_blank'); 
...
<div class="container">
    <br>
    <h3 align="center">ລາຍງານຜິດດ່ຽງຕົ້ນທືນຂາຍ</h3><br>
...
<table>
     ...
     <td> <br>  <a href="index.php"> <button type="button" name="reset" value="reset" class="btn btn-danger"><i class="fa fa-times"></i>&nbsp;ປິດ</button></a></td>
            ...
            <td>ວັນທີ<br><input type="date" class="form-control ss" name="from_date" id="from_date" value="<?php echo date("Y-m-d"); ?>"></td> 
            
            <td>ຫາ<br><input type="date" class="form-control ss" name="to_date" id="to_date" value="<?php echo date("Y-m-d"); ?>"></td> 
	...
	<?PHP } ?>
     ...
     <td>ລະຫັດສິນຄ້າ<br><input type="text" class="form-control" name="product_id" id="product_id" ></td>      
     <td>ລະຫັດລູກຄ້າ<br><input type="text" class="form-control" name="customer_id" id="customer_id" ></td> 
     <td>ເລກທີຂາຍ<br><input type="text" class="form-control" name="sale_id" id="sale_id" ></td> 
     <td>ພະນັກງານ<br>
	...
	while($f = mysqli_fetch_array($sql)){?>
		<option value="<?php echo $f['User_ID']?>"><?php echo $f['User_ID']?> &nbsp; <?php echo $f['User_Name']?></option>
	<?PHP } ?>
    </select> </td> 
