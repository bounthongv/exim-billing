	...
	,custoemr_sale_order.qty_p
	  ...
	  LEFT JOIN (select sum(product_sale.qty) as qty_p ,product_sale.sale_id,product_sale.sale_date
           from  product_sale 
     	          left join products on products.Product_ID=product_sale.product_id
		          left join tb_groups on tb_groups.Group_ID=products.group_id
              where 1=1 and tb_groups.Group_ID='001'  $btw $r_id $sr_id group by sale_id,sale_date) as custoemr_sale_order 
           ...
           <?php
			...
			?>	<tr>
			   <td align="center"><?=$s["sale_id"];?></td> 
                ...
                <td align="right"><?=@number_format($s["total_2"],2);?></td>
			...
			<?php	
             ...
             <td colspan="1" align="right"><?=@number_format($t_total_2,2);?></td>
