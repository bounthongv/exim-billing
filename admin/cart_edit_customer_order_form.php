...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "add")
	{
		...
		{
			$is_available = 0;
			foreach($_SESSION["cart_edit_customer_order"] as $keys => $values)
			{   
			
				if($_SESSION["cart_edit_customer_order"][$keys]['Product_ID'] == $_POST["Product_ID"])
				{
										
					$is_available++;
					if($_SESSION["cart_edit_customer_order"][$keys]['product_quantity'] == $_POST["qty_limit"])
					{  }
					else{
					
					$_SESSION["cart_edit_customer_order"][$keys]['product_quantity'] = $_SESSION["cart_edit_customer_order"][$keys]['product_quantity'] +                      $_POST["product_quantity"];	
					
					$_SESSION["cart_edit_customer_order"][$keys]['product_price'] =   $_POST["product_price"];								 
					}
				}
	...
	if($_POST["action"] == 'remove')
	{
		foreach($_SESSION["cart_edit_customer_order"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["cart_edit_customer_order"][$keys]);
			}
		}
	}
	...
	if($_POST["action"] == 'update')
	{
		foreach($_SESSION["cart_edit_customer_order"] as $keys => $values)
		{
			...
			if($_POST["product_quantity"]>$_SESSION["cart_edit_customer_order"][$keys]['product_quantity'])
			{
				$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_customer_order"][$keys]['qty_limit'];
				$_SESSION["cart_edit_customer_order"][$keys]['product_price']=$_POST["product_price"];
				}
				else{
			$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$_POST["product_quantity"];			
			$_SESSION["cart_edit_customer_order"][$keys]['product_price']=$_POST["product_price"];
				   }
			}
		}
	...
	if($_POST["action"] == 'update_x1')
	{
		foreach($_SESSION["cart_edit_customer_order"] as $keys => $values)
		{
			...
			if($_POST["qty"]>$_SESSION["cart_edit_customer_order"][$keys]['qty_limit'])
			{
				//$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_customer_order"][$keys]['qty_limit'];
				$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			    $_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$qty;
			
				$price = filter_var($_POST["price"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
				$_SESSION["cart_edit_customer_order"][$keys]['product_price']=$price;
				}
				else{
			$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$qty;	
...
for ($i = 0; $i < count($_POST['item_list']); $i++) {
    ...
    left join (select sum(qty) as stock_qty,product_id from stock_product where 1=1 and stock_id='0001' group by product_id ) as tb_stock_product
		...
		left join (select sum(qty) as order_qty,product_id from product_customer_order where 1=1 and status_sale='1' group by product_id ) as tb_product_order
              ...
              where    1=1 and products.Product_ID='$Product_ID'  order by products.Product_ID 
			    ");		 
	...
	if($_POST["action"] == 'update_plus')
	{
		foreach($_SESSION["cart_edit_customer_order"] as $keys => $values)
		{
			...
			$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']+1;
			/*if($_SESSION["cart_edit_customer_order"][$keys]['product_quantity']+1>$_SESSION["cart_edit_customer_order"][$keys]['qty_limit'])
			{
				$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=$_SESSION["cart_edit_customer_order"][$keys]['qty_limit'];
	...
	if($_POST["action"] == 'update_minus')
	{
		foreach($_SESSION["cart_edit_customer_order"] as $keys => $values)
		{
			...
			if($_SESSION["cart_edit_customer_order"][$keys]['product_quantity']-1<1)
			{
				$_SESSION["cart_edit_customer_order"][$keys]['product_quantity']=1;
