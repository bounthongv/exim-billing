...
if(isset($_POST["action"]))
{
	...
	{
		/// start 
		if(isset($_SESSION["cart_customer_order"]))
		{
			$is_available = 0;
			foreach($_SESSION["cart_customer_order"] as $keys => $values)
			{   
			
				if($_SESSION["cart_customer_order"][$keys]['Product_ID'] == $_POST["Product_ID"])
				{
										
					$is_available++;
					if($_SESSION["cart_customer_order"][$keys]['product_quantity'] == $_POST["qty_limit"])
					{  }
					else{
					
					$_SESSION["cart_customer_order"][$keys]['product_quantity'] = $_SESSION["cart_customer_order"][$keys]['product_quantity'] +                      $_POST["product_quantity"];	
					
					$_SESSION["cart_customer_order"][$keys]['product_price'] =   $_POST["product_price"];								 
					}
				}
					...
					'product_quantity'         =>     $_POST["product_quantity"]
						...
						$_SESSION["cart_customer_order"][] = $item_array;
	...
	if($_POST["action"] == 'remove')
	{
		foreach($_SESSION["cart_customer_order"] as $keys => $values)
		{
			...
			{
				unset($_SESSION["cart_customer_order"][$keys]);
			}
		}
	}
	...
	if($_POST["action"] == 'update')
	{
		foreach($_SESSION["cart_customer_order"] as $keys => $values)
		{
			...
			if($_POST["product_quantity"]>$_SESSION["cart_customer_order"][$keys]['product_quantity'])
			{
				$_SESSION["cart_customer_order"][$keys]['product_quantity']=$_SESSION["cart_customer_order"][$keys]['qty_limit'];
				$_SESSION["cart_customer_order"][$keys]['product_price']=$_POST["product_price"];
				}
				else{
			$_SESSION["cart_customer_order"][$keys]['product_quantity']=$_POST["product_quantity"];			
			$_SESSION["cart_customer_order"][$keys]['product_price']=$_POST["product_price"];
				   }
			}
		}
	...
	if($_POST["action"] == 'update_x1')
	{
		foreach($_SESSION["cart_customer_order"] as $keys => $values)
		{
			...
			if($_POST["qty"]>$_SESSION["cart_customer_order"][$keys]['qty_limit'])
			{
				//$_SESSION["cart_customer_order"][$keys]['product_quantity']=$_SESSION["cart_customer_order"][$keys]['qty_limit'];
				$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			    $_SESSION["cart_customer_order"][$keys]['product_quantity']=$qty;
			
				$price = filter_var($_POST["price"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
				$_SESSION["cart_customer_order"][$keys]['product_price']=$price;
				}
				else{
			$qty = filter_var($_POST["qty"], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);	
			$_SESSION["cart_customer_order"][$keys]['product_quantity']=$qty;	
...
for ($i = 0; $i < count($_POST['item_list']); $i++) {
	...
	if($_POST["action"] == 'update_plus')
	{
		foreach($_SESSION["cart_customer_order"] as $keys => $values)
		{
			...
			$_SESSION["cart_customer_order"][$keys]['product_quantity']=$_SESSION["cart_customer_order"][$keys]['product_quantity']+1;
			/*if($_SESSION["cart_customer_order"][$keys]['product_quantity']+1>$_SESSION["cart_customer_order"][$keys]['qty_limit'])
			{
				$_SESSION["cart_customer_order"][$keys]['product_quantity']=$_SESSION["cart_customer_order"][$keys]['qty_limit'];
	...
	if($_POST["action"] == 'update_minus')
	{
		foreach($_SESSION["cart_customer_order"] as $keys => $values)
		{
			...
			if($_SESSION["cart_customer_order"][$keys]['product_quantity']-1<1)
			{
				$_SESSION["cart_customer_order"][$keys]['product_quantity']=1;
